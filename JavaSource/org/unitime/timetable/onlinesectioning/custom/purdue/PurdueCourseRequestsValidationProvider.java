...
argument_list|)
expr_stmt|;
comment|//if (!ResponseStatus.success.name().equals(resp.status))
comment|//throw new SectioningException(resp.message == null || resp.message.isEmpty() ? "Failed to check student eligibility (" + resp.status + ")." : resp.message);
block|}
catch|catch
parameter_list|(
name|SectioningException
