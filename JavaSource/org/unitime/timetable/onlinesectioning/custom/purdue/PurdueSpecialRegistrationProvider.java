...
specifier|protected
name|String
name|getSpecialRegistrationDateFormat
parameter_list|()
block|{
return|return
name|ApplicationProperties
...
name|getProperty
argument_list|(
literal|"purdue.specreg.dateFormat"
argument_list|,
literal|"M-d-yyyy"
argument_list|)
return|;
...
argument_list|()
operator|+
literal|"/checkStudentModifyScheduleAttrs"
argument_list|)
return|;
block|}
specifier|protected
name|String
name|getSpecialRegistrationApiSiteUpdateRegistration
parameter_list|()
block|{
return|return
name|ApplicationProperties
...
name|getProperty
argument_list|(
literal|"purdue.specreg.site.updateRegistration"
argument_list|,
name|getSpecialRegistrationApiSite
argument_list|()
operator|+
literal|"/updateRequestorNotes"
argument_list|)
return|;
block|}
specifier|protected
...
literal|"UT_WAIT"
argument_list|,
literal|"Course is wait-listed, Wait-List override needs to be requested instead."
argument_list|)
argument_list|)
expr_stmt|;
name|ret
...
name|getProperty
argument_list|(
literal|"purdue.specreg.reg.requestorNoteSuggestions"
argument_list|,
literal|""
argument_list|)
operator|.
...
expr_stmt|;
block|}
comment|//request.requestorNotes = input.getNote();
if|if
condition|(
name|request
operator|.
...
name|inProgress
expr_stmt|;
comment|// if (r.requestorNotes == null) r.requestorNotes = input.getNote();
if|if
condition|(
name|r
operator|.
...
condition|)
continue|continue;
comment|// ignore wait-listed requests -> wait-list validator takes care of those
name|String
name|message
init|=
literal|""
...
block|}
block|}
comment|/* 					if (!drops.containsKey(course)&& !keeps.contains(course)&& maxStatus != null&& maxi != null&& clazz.getSchedulingSubpart().getParentSubpart() == null) { 						boolean first = true; 						for (SchedulingSubpart ss: clazz.getSchedulingSubpart().getInstrOfferingConfig().getSchedulingSubparts()) { 							if (ss.getParentSubpart() == null&& ss.getItype().getItype()< clazz.getSchedulingSubpart().getItype().getItype()) { first = false; break; } 						} 						if (first) { 							String message = maxi; 							switch (getStatus(maxStatus)) { 							case Approved: 								message = "Approved: " + message; 								if (excludeApprovedOrRejected) continue; 								break; 							case Rejected: 								message = "Denied: " + message; 								if (excludeApprovedOrRejected) continue; 								break; 							} 							if (maxiNote != null&& !maxiNote.toString().isEmpty()) 								message += "\n<span class='note'>" + maxiNote.trim() + "</span>"; 							String additionalMessage = ApplicationProperties.getProperty("purdue.specreg.note.MAXI." + 									(maxStatus == ChangeStatus.approved&& specialRequest.completionStatus == CompletionStatus.completed ? "completed" : maxStatus)); 							if (additionalMessage != null&& !additionalMessage.isEmpty()) { 								if (additionalMessage.contains("{course}")) 									additionalMessage = additionalMessage.replace("{course}", ca.getCourseName()); 								additionalMessages.add(additionalMessage); 							} 							 							if (maxStatus != null) 								message = "<span class='" + maxStatus + "'>" + message + "</span>"; 							ca.addError(message); 							if (ca.getSpecRegStatus() == null) 								ca.setSpecRegStatus(getStatus(maxStatus)); 							if (isPending(maxStatus)) 								ret.addError(new ErrorMessage(course.getCourseName(), "", "MAXI", maxi)); 						} 					} 					*/
name|ret
operator|.
name|addChange
argument_list|(
...
name|getProperty
argument_list|(
literal|"purdue.specreg.gm.requestorNoteSuggestions"
argument_list|,
literal|""
argument_list|)
operator|.
...
name|getProperty
argument_list|(
literal|"purdue.specreg.vt.requestorNoteSuggestions"
argument_list|,
literal|""
argument_list|)
operator|.
...
expr_stmt|;
block|}
comment|/* 					CourseOffering course = findCourseByExternalId(session.getUniqueId(), v.crn); 					if (course == null) continue; 					CourseCreditUnitConfig credit = course.getCredit(); 					if (credit == null) continue; 					if (credit instanceof VariableRangeCreditUnitConfig) { 						VariableRangeCreditUnitConfig vr = (VariableRangeCreditUnitConfig) credit; 						if (vr.isFractionalIncrementsAllowed()) { 							for (int c = Math.round(10f * credit.getMinCredit()); c<= Math.round(10f * credit.getMaxCredit()); c += 5) 								change.addAvailableCredit(0.1f * c); 						} else { 							for (int c = Math.round(credit.getMinCredit()); c<= Math.round(credit.getMaxCredit()); c ++) 								change.addAvailableCredit((float)c); 						} 					} else if (credit instanceof VariableFixedCreditUnitConfig) { 						change.addAvailableCredit(credit.getMinCredit()); 						change.addAvailableCredit(credit.getMaxCredit()); 					} else { 						continue; 					} 					*/
name|ret
operator|.
name|add
argument_list|(
...
name|message
operator|=
literal|"Variable Credit Change: "
operator|+
name|Formats
operator|.
name|getNumberFormat
...
control|)
block|{
comment|// if (r.requestorNotes == null) r.requestorNotes = request.getNote();
if|if
condition|(
name|r
operator|.
