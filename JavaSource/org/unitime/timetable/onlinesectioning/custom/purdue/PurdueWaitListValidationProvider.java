...
block|}
block|}
comment|// when enrolled, only continue when swap for enrolled course is enabled (section wait-list)
if|if
condition|(
name|enrolled
operator|!=
...
condition|)
continue|continue;
comment|// when enrolled, skip courses of lower choice than the enrollment
if|if
condition|(
name|enrolled
operator|!=
...
condition|)
continue|continue;
comment|// when enrolled, skip the enrolled course if the enrollment matches the requirements
if|if
condition|(
name|enrolled
operator|!=
...
block|}
block|}
comment|// no CRNs to check -> continue
if|if
condition|(
name|crns
operator|.
...
operator|+
literal|"registration in them please select Request Overrides. If you aren\u2019t sure, click Cancel Submit and "
operator|+
...
condition|)
block|{
comment|// Retrieve max credit request
name|SpecialRegistration
name|maxCreditReq
init|=
literal|null
decl_stmt|;
comment|// Try matching request first
if|if
condition|(
name|request
operator|.
...
block|}
block|}
comment|// Get latest not-cancelled max credit override request otherwise
if|if
condition|(
name|maxCreditReq
operator|==
...
name|getProperty
argument_list|(
literal|"purdue.specreg.waitlist.requestorNoteSuggestions"
argument_list|,
literal|""
argument_list|)
operator|.
...
condition|)
continue|continue;
comment|// when enrolled (section swap), check if active
if|if
condition|(
name|enrolledCourse
operator|!=
