...
argument_list|)
expr_stmt|;
comment|// Entity event = new Entity(-1l, "Event", "Event Rooms");
name|Entity
name|managed
init|=
operator|new
...
expr_stmt|;
block|}
comment|/* 			if (location.getEventDepartment() != null&& location.getEventDepartment().isAllowEvents()) { 				Set<String> roomTypes = eventRoomTypes.get(location.getEventDepartment().getUniqueId()); 				if (roomTypes != null&& roomTypes.contains(location.getRoomType().getLabel())) 					event.incCount(); 			} 			*/
if|if
condition|(
name|location
operator|.
...
expr_stmt|;
block|}
comment|// if (event.getCount()> 0)
comment|//	response.add("department", event);
if|if
condition|(
name|managed
operator|.
