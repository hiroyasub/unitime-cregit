...
block|}
block|}
comment|//		if (sessionHasCourseCatalog(toSession)){
comment|//			query = "select cc2 from CourseCatalog cc2";
comment|//			query += " where cc2.session.uniqueId=:sessionId";
comment|//			query += "  and cc2.uniqueId not in ";
comment|//			query += " (select distinct cc.uniqueId from CourseCatalog cc, CourseOffering co";
comment|//			query += "  where co.subjectArea.session.uniqueId=:sessionId";
comment|//			query += "  and co.subjectArea.subjectAreaAbbreviation=:subjectAbbv";
comment|//			query += "  and cc.session.uniqueId=:sessionId";
comment|//			query += "  and cc.subject=:subjectAbbv";
comment|//			query += "  and cc.courseNumber = co.courseNbr)";
comment|//			l = coDao.getQuery(query)
comment|//				.setString("subjectAbbv", subjectAreaAbbreviation)
comment|//				.setLong("sessionId", toSession.getUniqueId())
comment|//				.list();
comment|//			if (l != null){
comment|//				CourseCatalog cc = null;
comment|//				for (Iterator ccIt = l.iterator(); ccIt.hasNext();){
comment|//					cc = (CourseCatalog) ccIt.next();
comment|//					addInstructionalOffering(cc, toSession);
comment|//				}
comment|//			}
comment|//		}
name|coDao
...
try|try
block|{
comment|//			trns = hibSession.beginTransaction();
name|InstructionalOffering
