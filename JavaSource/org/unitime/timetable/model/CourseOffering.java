...
name|Exception
block|{
comment|/*         Transaction trans = null;         try {      	trans = hibSession.beginTransaction();       	hibSession.createQuery("update CourseOffering  c " +          		"set c.enrollment=(select count(distinct d.student) " +                  " from StudentClassEnrollment d " +                  " where d.courseOffering.uniqueId =c.uniqueId) " +                   " where c.subjectArea.uniqueId in (select sa.uniqueId " +                   " from SubjectArea sa where sa.session.uniqueId = :sessionId)").                  setLong("sessionId", acadSession.getUniqueId().longValue()).executeUpdate();          trans.commit();         } catch (Exception e) {      	   if (trans != null){      		   trans.rollback();      	   }  		   throw(e);         }         */
block|}
specifier|public
name|int
name|compareTo
