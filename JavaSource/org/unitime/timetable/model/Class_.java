...
name|createQuery
argument_list|(
literal|"select c.uniqueId from Class_ c inner join c.schedulingSubpart.instrOfferingConfig.instructionalOffering as io "
operator|+
...
expr_stmt|;
block|}
comment|/*     	// This does not work on MySQL (You can't specify target table 'CLASS_' for update in FROM clause)         hibSession.createQuery("update Class_  c " +         		"set c.enrollment=(select count(distinct d.student) " +                 " from StudentClassEnrollment d " +                 " where d.clazz.uniqueId =c.uniqueId) " +                  " where c.uniqueId in (select cls.uniqueId " +                  " from Class_ cls inner join cls.schedulingSubpart.instrOfferingConfig.instructionalOffering as io " +                 " where io.session.uniqueId = :sessionId)").                 setLong("sessionId", acadSession.getUniqueId().longValue()).executeUpdate();         */
name|trans
operator|.
name|commit
argument_list|()
