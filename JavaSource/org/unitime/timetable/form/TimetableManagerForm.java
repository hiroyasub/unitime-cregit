...
begin_comment
comment|/**  * Represents the Timetable Manager  *   * @author Heston Fernandes, Stephanie Schluttenhofer, Tomas Muller  */
end_comment
...
argument_list|)
expr_stmt|;
comment|/*             if(depts.size()==0) {                 Roles deptRole = Roles.getRole(Roles.DEPT_SCHED_MGR_ROLE);                 if (deptRole!=null&& roles.contains(deptRole.getRoleId().toString()))                     errors.add("depts",                             new ActionMessage("errors.generic", "At least one department must be assigned for role "+deptRole.getAbbv()));             }             */
if|if
condition|(
name|roles
operator|.
