...
literal|null
condition|)
comment|//&& OnlineSectioningService.getInstance(session.getUniqueId()) != null)
name|lastSessionId
operator|=
name|session
operator|.
...
name|createQuery
argument_list|(
literal|"select distinct c.uniqueId from CourseOffering c, TimetableManager m inner join m.departments d where "
operator|+
literal|"c.subjectArea.session.uniqueId = :sessionId and c.instructionalOffering.consentType is not null and "
operator|+
literal|"m.externalUniqueId = :extId and c.subjectArea.department = d"
argument_list|)
operator|.
name|setLong
argument_list|(
...
argument_list|()
operator|+
literal|") and (waitlist: true or consent: todo)"
expr_stmt|;
return|return
name|q
return|;
...
literal|null
operator|||
comment|// no role -> student itself
operator|!
name|user
operator|.
name|hasRole
...
condition|)
block|{
comment|// user is not advisor or advisor access is disabled
if|if
condition|(
name|status
operator|.
...
name|createQuery
argument_list|(
literal|"from OnlineSectioningLog where session.uniqueId = :sessionId and student = :studentExternalId order by timeStamp desc"
argument_list|)
operator|.
name|setLong
argument_list|(
