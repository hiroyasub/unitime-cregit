...
argument_list|)
expr_stmt|;
comment|/* 		if (!valid) { 			callback.onSuccess(false); 			return; 		} 		LoadingWidget.getInstance().show(MESSAGES.waitCheckingRoomAvailability()); 		RPC.execute(EventRoomAvailabilityRpcRequest.checkAvailability(iMeetings.getValue(), iSession.getAcademicSessionId()), new AsyncCallback<EventRoomAvailabilityRpcResponse>() { 			@Override 			public void onFailure(Throwable caught) { 				LoadingWidget.getInstance().hide(); 				callback.onFailure(caught); 			}  			@Override 			public void onSuccess(EventRoomAvailabilityRpcResponse result) { 				LoadingWidget.getInstance().hide(); 				iMeetings.setValue(result.getMeetings()); 				for (MeetingInterface meeting: result.getMeetings()) { 					if (meeting.hasConflicts()) { 						UniTimeNotifications.error(MESSAGES.reqNoOverlaps()); 						iHeader.setErrorMessage(MESSAGES.reqNoOverlaps()); 						callback.onSuccess(false); 						return; 					} 				} 				callback.onSuccess(true); 			} 		}); 		*/
block|}
specifier|protected
name|EventPropertiesRpcResponse
name|getProperties
