...
comment|//	private HashSet<PitClass> findAllPitClassesWithContactHoursForDepartmentsAndSubjectAreas(
comment|//			PointInTimeData pointInTimeData, Session hibSession) {
comment|//
comment|//		HashSet<PitClass> pitClasses = new HashSet<PitClass>();
comment|//
comment|//		for(Long deptId : getDepartmentIds()) {
comment|//			List<PitClass> pitClassesQueryResult = findAllPitClassesWithContactHoursForDepartment(pointInTimeData, deptId, hibSession);
comment|//			for(PitClass pc : pitClassesQueryResult) {
comment|//				if(pc.getPitSchedulingSubpart().getPitInstrOfferingConfig().getPitInstructionalOffering().getControllingPitCourseOffering().isIsControl().booleanValue()
comment|//&& getSubjectAreaIds().contains(pc.getPitSchedulingSubpart().getPitInstrOfferingConfig().getPitInstructionalOffering().getControllingPitCourseOffering().getSubjectArea().getUniqueId())) {
comment|//					pitClasses.add(pc);
comment|//				};
comment|//			}
comment|//		}
comment|//		return(pitClasses);
comment|//
comment|//	}
annotation|@
name|Override
