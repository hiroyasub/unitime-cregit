...
expr_stmt|;
block|}
comment|/* 	            DurationModel model = subpart.getInstrOfferingConfig().getDurationModel(); 	            for (Iterator i=subpart.getPreferences().iterator(); i.hasNext(); ) { 	                Preference pref = (Preference) i.next(); 	                if (pref instanceof TimePref&& !model.isValidCombination(mpw, subpart.effectiveDatePattern(), ((TimePref)pref).getTimePattern())) { 		                pref.setOwner(null); 		                hibSession.delete(pref); 		                i.remove(); 	                } 	            }  	            for (Iterator i=classes.iterator(); i.hasNext(); ) { 	                Class_ c = (Class_) i.next(); 		            Set cPrefs = c.getPreferences(); 		            for (Iterator j=cPrefs.iterator(); j.hasNext(); ) { 		                Preference pref = (Preference) j.next(); 		                if (pref instanceof TimePref&& !model.isValidCombination(mpw, c.effectiveDatePattern(), ((TimePref)pref).getTimePattern())) { 			                pref.setOwner(null); 			                hibSession.delete(pref); 			                j.remove(); 		                } 		            } 	                hibSession.saveOrUpdate(c); 	            } 	            */
