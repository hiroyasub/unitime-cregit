...
argument_list|)
throw|;
comment|/* for deletion 	        // backToInstrOffr - Go back to Instructional Offering Screen 	        if(op.equals(rsc.getMessage("button.backToInstrOffrDet"))&& classId!=null&& classId.trim().length()!=0 ) {  	            Class_DAO cdao = new Class_DAO(); 	            SchedulingSubpart ss = cdao.get(new Long(classId)).getSchedulingSubpart();  	            response.sendRedirect( response.encodeURL("instructionalOfferingDetail.do?op=view&io="+ss.getInstrOfferingConfig().getInstructionalOffering().getUniqueId())); 	        } 	        */
...
literal|null
operator|&&
comment|// HAS A COMMITED SOLUTION
name|c
operator|.
name|isEditableBy
argument_list|(
...
literal|null
operator|&&
comment|// NOT LOADED INTO THE SOLVER
name|c
operator|.
...
operator|)
operator|&&
comment|// NO SOLUTION IS SELECTED
name|user
operator|.
name|isAdmin
argument_list|()
comment|//TODO: remove this once the info box allows to touch only classes editable by the user
argument_list|)
expr_stmt|;
