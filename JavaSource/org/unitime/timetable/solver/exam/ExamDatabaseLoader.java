...
argument_list|()
expr_stmt|;
comment|// if (hasRoomAvailability()) loadRoomAvailability(RoomAvailability.getInstance());
name|loadExamsAllSection
argument_list|()
expr_stmt|;
comment|// loadExams();
comment|// loadStudents();
comment|// loadDistributions();
comment|// ExamType type = ExamTypeDAO.getInstance().get(iExamTypeId, hibSession);
comment|// if ("true".equals(ApplicationProperties.getProperty("tmtbl.exam.eventConflicts."+type.getReference(),"true"))) loadAvailabilitiesFromEvents();
comment|// if ("true".equals(ApplicationProperties.getProperty("tmtbl.exam.sameRoom."+type.getReference(),"false"))) makeupSameRoomConstraints();
name|getModel
argument_list|()
operator|.
name|init
...
name|periodPlacements
init|=
comment|// findPeriodsSoft(exam, type);
name|findPeriodsClass17Other8
argument_list|(
name|exam
argument_list|,
...
condition|)
block|{
comment|// iProgress.warn("Exam "+getExamLabel(exam)+" has no period available, it is not loaded.");
continue|continue;
block|}
name|Exam
name|x
...
argument_list|)
expr_stmt|;
comment|/*             if (exam.getAssignedPeriod()!=null) {                 boolean fail = false;                 ExamPeriod period = iPeriods.get(exam.getAssignedPeriod().getUniqueId());                 if (period==null) {                     iProgress.warn("Unable assign exam "+getExamLabel(exam)+" to period "+exam.getAssignedPeriod().getName()+": period not allowed.");                     fail = true;                 }                 ExamPeriodPlacement periodPlacement = (period==null?null:x.getPeriodPlacement(period));                  if (!fail&& periodPlacement==null) {                     iProgress.warn("Unable to assign exam "+getExamLabel(exam)+" to period "+exam.getAssignedPeriod().getName()+": period prohibited.");                     fail = true;                 }                 HashSet roomPlacements = new HashSet();                 if (!fail&& x.getMaxRooms()>0) {                     for (Iterator j=exam.getAssignedRooms().iterator();j.hasNext();) {                         Location location = (Location)j.next();                         ExamRoom room = iRooms.get(location.getUniqueId());                         if (room==null) {                             iProgress.warn("Unable to assign exam "+getExamLabel(exam)+" to room "+location.getLabel()+": not an examination room.");                             fail = true; break;                         } else {                             ExamRoomPlacement roomPlacement = x.getRoomPlacement(room);                             if (roomPlacement==null) {                                 iProgress.warn("Unable to assign exam "+getExamLabel(exam)+" to room "+location.getLabel()+": room not valid for this exam.");                                 fail = true; break;                             } else if (!roomPlacement.isAvailable(period)) {                                 iProgress.warn("Unable to assign exam "+getExamLabel(exam)+" to room "+location.getLabel()+": room not available at "+period+".");                                 fail = true; break;                             }                             roomPlacements.add(roomPlacement);                         }                     }                 }                 if (!fail&& roomPlacements.size()>x.getMaxRooms()) {                     iProgress.warn("Unable to assign exam "+getExamLabel(exam)+" to room"+(roomPlacements.size()>1?"s":"")+" "+roomPlacements+": number of assigned rooms exceeds the current limit ("+roomPlacements.size()+">"+x.getMaxRooms()+").");                     fail = true;                  }                 if (!fail)                     x.setInitialAssignment(new ExamPlacement(x, periodPlacement, roomPlacements));             }             */
block|}
block|}
specifier|protected
name|ExamInstructor
...
name|createQuery
argument_list|(
literal|"select e.student.uniqueId from StudentClassEnrollment e where e.clazz.uniqueId = :classId"
argument_list|)
operator|.
name|setLong
argument_list|(
...
name|createQuery
argument_list|(
literal|"select e.student.uniqueId from StudentClassEnrollment e where e.clazz.uniqueId = :classId and e.courseOffering.uniqueId = :courseId"
argument_list|)
operator|.
name|setLong
argument_list|(
